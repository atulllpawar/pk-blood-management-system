...
async function startServer() {
  ...
  app.post('/api/requests', (req, res) => {
    const { patientName, bloodGroup, hospital, city, units, urgency, contactMobile, notes } = req.body;
  ...
  app.post('/api/auth/login', (req, res) => {
    ...
    // Default admin credentials: admin / admin123 or pkadmin / pk123
    if (
  ...
  app.post('/api/ai/match', async (req, res) => {
    ...
    // Find direct matching approved donors
...
Provide a concise 3-bullet medical advice for blood bank coordination, compatibility guidance, and rapid donor outreach steps. Keep it professional, reassuring, and clear.`;
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`PK Blood Management Server running on http://localhost:${PORT}`);
  });
}
